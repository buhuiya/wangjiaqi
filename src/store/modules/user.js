...
import {login, getUserInfo, getUserAgent, logoutUser} from "@/api/sys";
import {
    getCurrentUserInfo,
    updateUserInfo,
    getAccessPath,
...
export default {
    ...
    mutations: {
        ...
        setAllUserCount(state, allUserCount) {
    ...
    actions: {
        ...
        // 获取所有用户数量
        async getAllUserCount(context) {
            // 获取服务器端返回的数据
            const res = await getAllUserCount();
            this.commit("user/setAllUserCount", res);
            return res;
        },
