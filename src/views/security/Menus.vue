<template>
  <div class="menu-container">
    <el-card shadow="never">
      ...
      <div class="menu-body">
        <el-table v-if="refreshTable" :data="menuList"
            :header-cell-style="{ background: '#f8f8f9', color: '#606266' }"
            ref="table" row-key="id" style="width: 100%" size="medium" :default-expand-all="isExpandAll">
...
import AddMenu from "@/views/security/components/AddMenu";
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      ...
      let option = JSON.parse(JSON.stringify(this.menuList));
      option.forEach(item => {
        if (item.children.length >= 0) this.$delete(item, 'children')
      })
      option.push({
        "id": 1,
        "path": "/",
      ...
      option = option.filter((o)=>{
        return o.name !== '仪表盘' &&  o.name !== '个人中心';
      })
      this.$refs.addMenu.options = option;
    },
    handleQuery() {
      this.getTableData();
    },
